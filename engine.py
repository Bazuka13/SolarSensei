...
def get_gemini_insights(location, system_size, savings, cost, roi, flux):
    # Abhi ke liye backup use kar rahe hain taaki error na aaye
...
def get_solar_analysis(lat, lon, bill, area, location_name="India"):
    ...
    if not NREL_KEY: 
        print("Warning: NREL Key missing. Using Mock Data.")
        # Mocking data so app doesn't crash without key
        avg_flux = 5.5
        annual_gen = int(estimate_system_size(bill, area) * 1400)
        ...
        try:
            ...
            return {"success": False, "error": f"NREL API Error: {str(e)}"}
    ...
    yield_per_kw = annual_savings / system_capacity if system_capacity > 0 else 0
    sensitivity_data = {
        "sizes": [3, 5, 8, 10],
        "savings": [int(3*yield_per_kw), int(5*yield_per_kw), int(8*yield_per_kw), int(10*yield_per_kw)]
    }

    user_yield = annual_gen / system_capacity if system_capacity > 0 else 0
    comparison = [
    ...
    # --- FIX: Generate Flat List of Insights ---
    # Frontend expects a list of strings called 'ai_insights'
    ...
    return {
        ...
        # --- KEY CHANGE HERE ---
        # Pehle ye "insights": {"pros":.., "cons":..} tha
        # Ab ye "ai_insights": [list of strings] hai, jo JS chahta hai
